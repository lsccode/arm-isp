        ...
        cmos_move_exposure_history( (cmos_fsm_ptr_t)p_fsm );
        {
            ...
            if ( ( wdr_mode == WDR_MODE_FS_LIN ) || ( ( wdr_mode == WDR_MODE_NATIVE ) && ( sensor_info.sensor_exp_number == 3 ) ) ) {
                if ( sensor_info.sensor_exp_number == 1 ) {
                    ...
                    //LOG( LOG_NOTICE, " short %d medium %d long %d Rs %d Rm %d", exp_set.integration_time, exp_set.integration_time_medium, exp_set.integration_time_long, exp_set.exposure_ratio_short, exp_set.exposure_ratio_medium );
    ...
    if ( ( wdr_mode == WDR_MODE_FS_LIN ) || ( ( wdr_mode == WDR_MODE_NATIVE ) && ( sensor_info.sensor_exp_number == 3 ) ) ) {
        ...
        *p_fsm->long_it_hist.p = integration_time_long_quant;
        ...
        if ( sensor_info.sensor_exp_number == 4 ) {
            ...
            time.int_time_L = p_fsm->integration_time_long;
            //LOG( LOG_NOTICE, " short %d medium %d long %d", p_fsm->integration_time_short, p_fsm->integration_time_medium, p_fsm->integration_time_long );
