...
void *get_sbuf_addr( enum sbuf_type buf_type, uint32_t buf_idx, struct fw_sbuf *p_fw_sbuf )
{
    ...
    switch ( buf_type ) {
#if ISP_HAS_AE_BALANCED_FSM
    case SBUF_TYPE_AE:
        buf = &( p_fw_sbuf->ae_sbuf[buf_idx] );
        break;
#endif

#if ( ISP_HAS_AWB_MESH_FSM || ISP_HAS_AWB_MESH_NBP_FSM )
    case SBUF_TYPE_AWB:
        buf = &( p_fw_sbuf->awb_sbuf[buf_idx] );
        break;
#endif

#if ISP_HAS_AF_LMS_FSM
    case SBUF_TYPE_AF:
        buf = &( p_fw_sbuf->af_sbuf[buf_idx] );
        break;
#endif

#if ISP_HAS_GAMMA_CONTRAST_FSM
    case SBUF_TYPE_GAMMA:
        buf = &( p_fw_sbuf->gamma_sbuf[buf_idx] );
        break;
#endif

#if ISP_HAS_IRIDIX_HIST_FSM || ISP_HAS_IRIDIX8_FSM
...
static int update_stats_ae( user2kernel_fsm_ptr_t p_fsm, uint32_t ae_buf_idx )
{
    ...
    if ( !p_sbuf_ae ) {
        LOG( LOG_ERR, "AE --- Fatal error: get ae sbuf failed, idx: %u, addr: %p.", ae_buf_idx, p_sbuf_ae );
        return -1;
    }
...
static void update_result_ae( user2kernel_fsm_ptr_t p_fsm, uint8_t ae_idx )
{
    ...
    if ( !p_sbuf_ae ) {
        LOG( LOG_ERR, "Fatal error: get ae sbuf failed, idx: %u.", ae_idx );
        return;
    }
    ...
    LOG( LOG_INFO, "Updating AE: ctx: %d, AE exposure_log2: %d, exposure_ratio: %u.", fw_id, p_sbuf_ae->ae_exposure, p_sbuf_ae->ae_exposure_ratio );
...
static int update_stats_awb( user2kernel_fsm_ptr_t p_fsm, uint32_t awb_buf_idx )
{
    ...
    if ( !p_sbuf_awb ) {
        LOG( LOG_ERR, "AWB --- Fatal error: get awb sbuf failed, idx: %u, addr: %p.", awb_buf_idx );
        return -1;
    }
...
static void update_result_awb( user2kernel_fsm_ptr_t p_fsm, uint8_t awb_idx )
{
    ...
    if ( !p_sbuf_awb ) {
        LOG( LOG_ERR, "Fatal error: get awb sbuf failed, idx: %u.", awb_idx );
        return;
    }
    ...
    LOG( LOG_INFO, "Updating AWB: ctx: %d, red_gain: %u, blue_gain: %u, p_high: %d, temperature_detected: %d, light_src_candidate: %d.",
         fw_id,
...
static int update_stats_af( user2kernel_fsm_ptr_t p_fsm, uint32_t af_buf_idx )
{
    ...
    if ( !p_sbuf_af ) {
        LOG( LOG_ERR, "AF --- Fatal error: get AF sbuf failed, idx: %u, addr: %p.", af_buf_idx );
        return -1;
    }
...
static void update_result_af( user2kernel_fsm_ptr_t p_fsm, uint8_t af_idx )
{
    ...
    if ( !p_sbuf_af ) {
        LOG( LOG_ERR, "Fatal error: get af sbuf failed, idx: %u.", af_idx );
        return;
    }
...
static int update_stats_gamma( user2kernel_fsm_ptr_t p_fsm, uint32_t gamma_buf_idx )
{
    ...
    if ( !p_sbuf_gamma ) {
        LOG( LOG_ERR, "Gamma --- Fatal error: get Gamma sbuf failed, idx: %u, addr: %p.", gamma_buf_idx );
        return -1;
    }
...
static void update_result_gamma( user2kernel_fsm_ptr_t p_fsm, uint8_t gamma_idx )
{
    ...
    if ( !p_sbuf_gamma ) {
        LOG( LOG_ERR, "Fatal error: get gamma sbuf failed, idx: %u.", gamma_idx );
        return;
    }
    ...
    LOG( LOG_INFO, "Updating gamma: ctx: %d, gamma gain: %u, offset: %u.",
         fw_id,
...
static void update_result_iridix_hist( user2kernel_fsm_ptr_t p_fsm, uint8_t iridix_idx )
{
    ...
    if ( !p_sbuf_iridix ) {
        LOG( LOG_ERR, "Fatal error: get iridix sbuf failed, idx: %u.", iridix_idx );
        return;
    }
...
#if defined( ISP_HAS_IRIDIX8_FSM )
    ...
    LOG( LOG_INFO, "Updating iridix: ctx: %d, iridix_strength: %u.", fw_id, p_sbuf_iridix->strength_target );
...
int user2kernel_initialize( user2kernel_fsm_ptr_t p_fsm )
{
    ...
    if ( p_ctx->fd_dev != -1 ) {
        LOG( LOG_NOTICE, "ISP device successfully opened %s p_ctx->fd_dev:0x%x", p_ctx->dev_path, p_ctx->fd_dev );
    } else {
        LOG( LOG_ERR, "Failed to open device %s", p_ctx->dev_path );
        return -2;
    }
    ...
    if ( p_ctx->map_base == (void *)MAP_FAILED ) {
        LOG( LOG_ERR, "sbuf map failed, err: %s.", strerror( errno ) );
        close( p_ctx->fd_dev );
        p_ctx->fd_dev = -1;
        return -3;
    } else {
...
void user2kernel_deinit( user2kernel_fsm_ptr_t p_fsm )
{
    ...
    if ( fw_id >= acamera_get_context_number() ) {
        LOG( LOG_ERR, "Fatal error: Invalid FW context ID: %d, max is: %d", fw_id, acamera_get_context_number() - 1 );
        return;
    }
...
uint8_t user2kernel_get_calibrations( uint32_t ctx_id, void *sensor_arg, ACameraCalibrations *c )
{
    ...
    LOG( LOG_DEBUG, "+++ get_calibration for ctx_id: %d.", ctx_id );
    ...
    LOG( LOG_INFO, "p_ctx: %p, cali_base: %p, cali_data_base: %p, CALIBRATION_TOTAL_SIZE: %d.", p_ctx, cali_base, cali_data_base, CALIBRATION_TOTAL_SIZE );
    ...
    for ( idx = 0; idx < CALIBRATION_TOTAL_SIZE; idx++ ) {
        ...
        if ( idx <= 5 || idx >= ( CALIBRATION_TOTAL_SIZE - 5 ) ) {
            LOG( LOG_DEBUG, "cali_lut_base[%d].ptr: %p, rows: %d, cols: %d, width: %d.",
                 idx,
    ...
    LOG( LOG_DEBUG, "--- get_calibration done for ctx_id: %d", ctx_id );
