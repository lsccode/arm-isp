...
{
    LOG( LOG_INFO, "I2C bus init for bus 0x%x", bus );
    ...
    if ( virt_addr == NULL ) {
        if ( bus_number < FPGA_IIC_MASTERS_MAX ) {
            ...
            if ( virt_addr != NULL ) {
                ...
                LOG( LOG_INFO, "I2C connection has been opened" );
            } else {
...
uint8_t system_i2c_write( uint32_t bus, uint32_t phy_addr, uint8_t *data, uint32_t size )
{
    ...
    if ( virt_addr != NULL ) {
        ...
        if ( no_stop != 0 ) {
            ...
            read_size = size;
            LOG( LOG_DEBUG, "Save i2c read phy_addr 0x%x, size %d", read_phy_addr, read_size );
            return I2C_OK;
        }
    ...
    if ( virt_addr != NULL ) {
        ...
        if ( result == 0 ) {
            ...
            if ( read_size != 0 ) {
                ...
                uint32_t idx = 0;
                LOG( LOG_DEBUG, "Write read phy_addr 0x%x, size %d", read_phy_addr, read_size );
                iowrite32( start_seq, virt_addr + FPGA_IIC_TX_FIFO );
                for ( idx = 0; idx < read_size; idx++ ) {
                    ...
                    LOG( LOG_DEBUG, "Write 0x%x", addr_byte );
                }
                read_size = 0;
            }
            ...
            for ( idx = 0; idx < size; idx++ ) {
                ...
                if ( rx_fifo_has_data( virt_addr ) != 1 ) {
                    LOG( LOG_DEBUG, "Failed to get data from FIFO" );
                };
