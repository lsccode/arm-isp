...
static void lens_print_params( void *ctx )
{
    ...
    LOG( LOG_NOTICE, "SOC SENSOR PARAMETERS" );
    LOG( LOG_NOTICE, "lens_type: %d", p_ctx->param.lens_type );
    LOG( LOG_NOTICE, "min_step: %d", p_ctx->param.min_step );
    LOG( LOG_NOTICE, "next_zoom: %d", p_ctx->param.next_zoom );
    LOG( LOG_NOTICE, "curr_zoom: %d", p_ctx->param.curr_zoom );
    LOG( LOG_NOTICE, "next_pos: %d", p_ctx->param.next_pos );
}
...
void lens_v4l2_subdev_deinit( void *ctx )
{
    ...
    if ( p_ctx != NULL ) {
        ...
        if ( --ctx_counter < 0 )
            LOG( LOG_ERR, "Lens context pointer is negative %d", ctx_counter );
        if ( rc != 0 ) {
            LOG( LOG_ERR, "Failed to reset soc_lens. core->reset failed with rc %d", rc );
            return;
        }
