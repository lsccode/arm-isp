...
static int sbuf_mgr_alloc_sbuf( struct sbuf_mgr *p_sbuf_mgr )
{
    ...
    LOG( LOG_NOTICE, "sbuf: len_needed: %zu, len_alloc: %u, len_used: %u, page_size: %lu, buf_alloc: %p, buf_used: %p.",
...
static uint32_t get_cur_calibration_total_size( void *fw_instance )
{
    ...
    LOG( LOG_NOTICE, "fw_instance: %p, CALIBRATION_TOTAL_SIZE: %d.", fw_instance, CALIBRATION_TOTAL_SIZE );
...
static int update_cur_calibration_to_sbuf( void *fw_instance, struct sbuf_mgr *p_sbuf_mgr )
{
    ...
    LOG( LOG_INFO, "sbuf_cali_base: %p, p_sbuf_lut_arr: %p, p_sbuf_cali_data: %p", sbuf_cali_base, p_sbuf_lut_arr, p_sbuf_cali_data );
    ...
    for ( idx = 0; idx < CALIBRATION_TOTAL_SIZE; idx++ ) {
        ...
        if ( !p_lut->ptr ) {
            rc = -1;
            LOG( LOG_ERR, "IQ LUT %d is NULL", idx );
            break;
        }
...
static uint32_t sbuf_mgr_item_count_in_using( struct sbuf_mgr *p_sbuf_mgr )
{
    ...
    LOG( LOG_INFO, "sbuf item using total count: %u.", rc );
...
static int sbuf_calibration_init( struct sbuf_context *p_ctx )
{
    ...
    LOG( LOG_DEBUG, "sbuf_calibration_init." );
    ...
    while ( !sbuf_calibration_is_ready_to_update( p_ctx ) ) {
        LOG( LOG_NOTICE, "wait for UF to finish using" );
        // sleep 3 ms
        ...
        // 3 seconds timeout
        if ( cnt >= 1000 ) {
            LOG( LOG_NOTICE, "timeout to wait for calibration ready to update" );
            p_ctx->p_fsm->is_paused = 0;
            return -1;
        }
    }
    ...
    LOG( LOG_NOTICE, "sbuf_calibration wdr_mode updated: %d -> %d.", p_ctx->sbuf_mgr.cur_wdr_mode, wdr_mode );
...
void sbuf_update_calibration_data( sbuf_fsm_ptr_t p_fsm )
{
    ...
    if ( p_ctx->fw_id != fw_id ) {
        LOG( LOG_ERR, "Error: ctx_id not match, fsm fw_id: %d, ctx_id: %d.", fw_id, p_ctx->fw_id );
        return;
    }
...
static void sbuf_mgr_reset( struct sbuf_mgr *p_sbuf_mgr )
{
    ...
    for ( i = 0; i < SBUF_STATS_ARRAY_SIZE; i++ ) {
        ...
        if ( i == SBUF_STATS_ARRAY_SIZE - 1 ) {
            LOG( LOG_INFO, "iridix sbuf arr info: read_idx: %u, write_idx: %u, status_count: %u-%u-%u-%u.",
...
void sbuf_update_ae_idx( sbuf_fsm_t *p_fsm )
{
    ...
    if ( sbuf_get_item( p_ctx->fw_id, &sbuf ) ) {
        LOG( LOG_INFO, "Failed to get sbuf for ae done buffer, fw_id: %d.", p_ctx->fw_id );
        return;
    }
    ...
    if ( sbuf_get_item( p_ctx->fw_id, &sbuf_iridix ) ) {
        LOG( LOG_INFO, "Failed to get sbuf for iridix done buffer, fw_id: %d.", p_ctx->fw_id );
...
void sbuf_update_awb_idx( sbuf_fsm_t *p_fsm )
{
    ...
    if ( sbuf_get_item( p_ctx->fw_id, &sbuf ) ) {
        LOG( LOG_INFO, "Failed to get sbuf for awb done buffer, fw_id: %d.", p_ctx->fw_id );
        return;
    }
...
void sbuf_update_af_idx( sbuf_fsm_t *p_fsm )
{
    ...
    if ( sbuf_get_item( p_ctx->fw_id, &sbuf ) ) {
        LOG( LOG_INFO, "Failed to get sbuf for af done buffer, fw_id: %d.", p_ctx->fw_id );
        return;
    }
...
void sbuf_update_gamma_idx( sbuf_fsm_t *p_fsm )
{
    ...
    if ( sbuf_get_item( p_ctx->fw_id, &sbuf ) ) {
        LOG( LOG_INFO, "Failed to get sbuf for gamma done buffer, fw_id: %d.", p_ctx->fw_id );
        return;
    }
...
static void sbuf_mgr_apply_new_param( struct sbuf_context *p_ctx, struct sbuf_idx_set *p_idx_set )
{
    ...
    if ( p_idx_set->awb_idx_valid && ( p_idx_set->awb_idx < SBUF_STATS_ARRAY_SIZE ) ) {
        ...
        LOG( LOG_INFO, "ctx: %d, AWB param: awb_red_gain: %u, awb_blue_gain: %u, temperature: %d, light_source: %u, p_high: %u",
             p_ctx->fw_id,
    ...
    if ( p_idx_set->af_idx_valid && ( p_idx_set->af_idx < SBUF_STATS_ARRAY_SIZE ) ) {
        ...
        LOG( LOG_INFO, "ctx: %d, AF pos: %u.", p_ctx->fw_id, p_sbuf_af->af_position );
    ...
    if ( p_idx_set->gamma_idx_valid && ( p_idx_set->gamma_idx < SBUF_STATS_ARRAY_SIZE ) ) {
        ...
        LOG( LOG_INFO, "ctx: %d, gamma param: gain: %u, offset: %u.", p_ctx->fw_id, p_sbuf_gamma->gamma_gain, p_sbuf_gamma->gamma_offset );
    ...
    if ( p_idx_set->iridix_idx_valid && ( p_idx_set->iridix_idx < SBUF_STATS_ARRAY_SIZE ) ) {
        ...
        LOG( LOG_INFO, "ctx: %d, iridix strength: %u, iridix_dark_enh: %u.", p_ctx->fw_id, p_sbuf_iridix->strength_target, p_sbuf_iridix->iridix_dark_enh );
...
static int sbuf_get_item_from_arr( struct sbuf_mgr *p_sbuf_mgr, struct sbuf_item *item, struct sbuf_item *arr, struct sbuf_item_arr_info *info )
{
    ...
    if ( SBUF_STATUS_DATA_EMPTY == item->buf_status ) {
        ...
        if ( ( info->item_status_count[SBUF_STATUS_DATA_USING] == 0 ) &&
            ...
            LOG( LOG_INFO, "Failed to get done sbuf, using count: %u, done count: %u, read_idx: %u, read_idx status: %u",
                 info->item_status_count[SBUF_STATUS_DATA_USING],
                 info->item_status_count[SBUF_STATUS_DATA_DONE],
                 info->read_idx,
    ...
    if ( rc ) {
        LOG( LOG_INFO, "Get sbuf item failed: buf_type: %s, buf_status: %s.", sbuf_type_str[item->buf_type], sbuf_status_str[item->buf_status] );
    } else {
...
static int sbuf_fops_open( struct inode *inode, struct file *f )
{
    ...
    if ( !p_ctx ) {
        LOG( LOG_CRIT, "Error, sbuf contexts is crashed, contents dump:" );
        for ( i = 0; i < acamera_get_context_number(); i++ ) {
            p_ctx = &sbuf_contexts[i];
            LOG( LOG_ERR, "%d): fw_id: %d, minor_id: %d, name: %s, p_fsm: %p.",
...
static int sbuf_fops_mmap( struct file *file, struct vm_area_struct *vma )
{
    ...
    if ( ( user_buf_len != sizeof( struct fw_sbuf ) ) &&
        ...
        LOG( LOG_ERR, "Not matched buf size, User app size: %ld, kernel sbuf size: %zu.", user_buf_len, sizeof( struct fw_sbuf ) );
        return -EINVAL;
    }
...
void sbuf_fsm_initialize( sbuf_fsm_t *p_fsm )
{
    ...
    if ( fw_id >= acamera_get_context_number() ) {
        LOG( LOG_CRIT, "Invalid FW context ID: %d, max is: %d", fw_id, acamera_get_context_number() - 1 );
        return;
    }
