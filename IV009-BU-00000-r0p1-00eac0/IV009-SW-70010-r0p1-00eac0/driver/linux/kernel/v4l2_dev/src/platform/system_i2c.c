...
{
    LOG( LOG_NOTICE, "I2C bus init for bus 0x%x", bus );
    ...
    if ( virt_addr == NULL ) {
        if ( bus_number < FPGA_IIC_MASTERS_MAX ) {
            ...
            if ( virt_addr != NULL ) {
                ...
                LOG( LOG_NOTICE, "I2C connection has been opened" );
            } else {
...
static int32_t iic_bus_is_ready( void *virt_addr )
{
    ...
    while ( ready == 0 ) {
        ...
        jt = jiffies;
        if ( ( jt >= js ? ( jt - js ) : ( ( uint32_t )( ~0 ) - js + jt ) ) > FPGA_IIC_TIMEOUT_HZ ) {
            result = -1;
...
uint8_t system_i2c_write( uint32_t bus, uint32_t phy_addr, uint8_t *data, uint32_t size )
{
    ...
    if ( virt_addr != NULL ) {
        ...
        if ( no_stop != 0 ) {
            ...
            read_size = size;
            LOG( LOG_NOTICE, "Save i2c read phy_addr 0x%x, size %d", read_phy_addr, read_size );
            return I2C_OK;
        }
        ...
        if ( result == 0 ) {
            ...
            LOG( LOG_ERR, "I2C write failed. No connect for the bus 0x%x ", bus );
            result = I2C_NOCONNECT;
            iowrite32( 2, virt_addr + FPGA_IIC_CR );
            iowrite32( 1, virt_addr + FPGA_IIC_CR );
        }
    } else {
        LOG( LOG_ERR, "IIC virtual phy_addr for bus 0x%x was not found", bus );
        result = I2C_NOCONNECT;
    }
...
uint8_t system_i2c_read( uint32_t bus, uint32_t phy_addr, uint8_t *data, uint32_t size )
{
    ...
    if ( virt_addr != NULL ) {
        LOG( LOG_DEBUG, "I2C read phy_addr 0x%x  size %d", addr, size );
        uint32_t idx = 0;
        // start sequence
        uint32_t start_seq = 0x00000100 + addr;
        ...
        if ( result == 0 ) {
            LOG( LOG_DEBUG, "Reading 0x%x", start_seq );
            ...
            if ( read_size != 0 ) {
                ...
                uint32_t idx = 0;
                LOG( LOG_INFO, "Write read phy_addr 0x%x, size %d", read_phy_addr, read_size );
                iowrite32( start_seq, virt_addr + FPGA_IIC_TX_FIFO );
                for ( idx = 0; idx < read_size; idx++ ) {
                    ...
                    LOG( LOG_INFO, "Write 0x%x", addr_byte );
                }
                read_size = 0;
            }
            ...
            for ( idx = 0; idx < size; idx++ ) {
                ...
                LOG( LOG_DEBUG, "Reading wait for rx fifo " );
                if ( rx_fifo_has_data( virt_addr ) != 1 ) {
                    LOG( LOG_INFO, "Failed to get data from FIFO" );
                    //Flashed the FIFO
                    // reset
                    iowrite32( 2, virt_addr + FPGA_IIC_CR );
                    iowrite32( 1, virt_addr + FPGA_IIC_CR );
                ...
                *data = ioread32( virt_addr + FPGA_IIC_RX_FIFO );
                LOG( LOG_DEBUG, "I2C read %d finished. val is 0x%x", idx, *data );
                data++;
            }
        } else {
            result = I2C_NOCONNECT;
