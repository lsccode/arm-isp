...
//static uint8_t *ext_config = NULL;
...
int32_t system_spi_init( void )
{
    ...
    LOG( LOG_INFO, "INIT SPI SYSTEM - virt address 0x%x", p_base );
    if ( p_base != NULL ) {
        base_valid = 1;

        init_spi_core();

        LOG( LOG_NOTICE, "SPI bus has been initialized" );
    } else {
...
uint32_t system_spi_rw32( uint32_t sel_mask, uint32_t control, uint32_t data, uint8_t data_size )
{
    ...
    if ( !( control & TX_NEG_MSK ) )
        ...
        ctrl &= ~( SPI_CR_CLK_POLARITY_MASK | SPI_CR_CLK_PHASE_MASK );
    ctrl |= SPI_CR_TXFIFO_RESET_MASK | SPI_CR_RXFIFO_RESET_MASK;
    SPI_WRITE32( FPGA_SPI_SPICR, ctrl );
    ...
    data = 0;
    for ( i = 0; i < size; i++ ) {
        data |= ( ( SPI_READ32( FPGA_SPI_DRR ) & 0xff ) << ( i * 8 ) );
    }
